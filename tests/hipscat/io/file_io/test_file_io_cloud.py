...
def test_write_string_to_file(tmp_cloud_path, file_system, storage_options):
...
def test_load_json(small_sky_dir_local, small_sky_dir_cloud, file_system, storage_options):
    ...
    json_dict_cloud = load_json_file(
        catalog_cloud_path, file_system=file_system, storage_options=storage_options
    )
    json_dict_local = load_json_file(catalog_info_pointer, encoding="utf-8")
    assert json_dict_cloud == json_dict_local


def test_load_parquet_to_pandas(small_sky_dir_local, small_sky_dir_cloud, file_system, storage_options):
    ...
    loaded_df = load_parquet_to_pandas(
        pixel_data_path_cloud, file_system=file_system, storage_options=storage_options
    )
    pd.testing.assert_frame_equal(parquet_df, loaded_df)


def test_write_df_to_csv(tmp_cloud_path, file_system, storage_options):
    ...
    loaded_df = load_csv_to_pandas(
        test_file_pointer, file_system=file_system, storage_options=storage_options
    )
