...
def test_write_catalog_info(
    ...
    with TempCloudDirectory(tmp_dir_cloud, "write_catalog_info", example_cloud_storage_options) as temp_path:
        ...
        assert_text_file_matches(
            expected_lines, metadata_filename, storage_options=example_cloud_storage_options
        )
...
def test_write_parquet_metadata(
    ...
    with TempCloudDirectory(
        tmp_dir_cloud, "write_parquet_metadata", example_cloud_storage_options
    ) as temp_path:
        catalog_base_dir = temp_path
...
def test_read_write_fits_point_map(tmp_dir_cloud, example_cloud_storage_options):
    ...
    with TempCloudDirectory(tmp_dir_cloud, "write_fits", example_cloud_storage_options) as temp_path:
        initial_histogram = hist.empty_histogram(1)
        filled_pixels = [51, 29, 51, 0]
        initial_histogram[44:] = filled_pixels[:]
        io.write_fits_map(temp_path, initial_histogram, storage_options=example_cloud_storage_options)
