...
def test_run_index(
    ...
    with TempCloudDirectory(tmp_dir_cloud, "run_index", example_cloud_storage_options) as temp_path:
        args = IndexArguments(
        ...
        catalog = Dataset.read_from_hipscat(args.catalog_path, storage_options=example_cloud_storage_options)
        ...
        schema = read_parquet_metadata(
            outfile, storage_options=example_cloud_storage_options
        ).schema.to_arrow_schema()
        assert schema.equals(basic_index_parquet_schema, check_metadata=False)

        schema = read_parquet_metadata(
            os.path.join(args.catalog_path, "_metadata"), storage_options=example_cloud_storage_options
        ).schema.to_arrow_schema()
        assert schema.equals(basic_index_parquet_schema, check_metadata=False)

        schema = read_parquet_metadata(
            os.path.join(args.catalog_path, "_common_metadata"), storage_options=example_cloud_storage_options
        ).schema.to_arrow_schema()
        assert schema.equals(basic_index_parquet_schema, check_metadata=False)
